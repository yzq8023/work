...
import com.service.service.utils.DiffUtils;
...
public abstract class ITicketService implements IManager {
	...
	private static class TicketKey {
		...
		TicketKey(TaskEntity repository, long ticketId) {
			this.repository = repository.getTaskName();
...
//			log.info("Built Lucene index over all tickets in {} secs", duration);
	 ...
	 * @since 1.4.0
	 */
	public boolean isAcceptingNewPatchsets(TaskEntity repository) {
		return isReady()
...
//				&& repository.acceptNewPatchsets
	 ...
	 * @since 1.4.0
	 */
	public boolean isAcceptingNewTickets(TaskEntity repository) {
		return isReady()
...
//				&& repository.acceptNewTickets
	 ...
	 * @since 1.4.0
	 */
	public boolean isAcceptingTicketUpdates(TaskEntity repository) {
		return isReady()
	 ...
	 * @since 1.4.0
	 */
	public final synchronized void resetCaches(TaskEntity repository) {
		List<TicketKey> repoKeys = new ArrayList<TicketKey>();
		for (TicketKey key : ticketsCache.asMap().keySet()) {
			if (key.repository.equals(repository.getCrtName())) {
				repoKeys.add(key);
			}
		}
		ticketsCache.invalidateAll(repoKeys);
		labelsCache.remove(repository.getTaskName());
		milestonesCache.remove(repository.getTaskName());
	 ...
	 * @since 1.4.0
	 */
	protected abstract void resetCachesImpl(TaskEntity repository);
	 ...
	 * @since 1.4.0
	 */
	public List<TicketLabel> getLabels(TaskEntity repository) {
		String key = repository.getTaskName();
	 ...
	 * @since 1.4.0
	 */
	public TicketLabel getLabel(TaskEntity repository, String label) {
	 ...
	 * @since 1.4.0
	 */
	public synchronized TicketLabel createLabel(TaskEntity repository, String label, String createdBy) {
		TicketLabel lb = new TicketMilestone(label);
		Repository db = null;
		try {
			db = repositoryManager.getRepository(repository.getTaskName());
			StoredConfig config = db.getConfig();
	 ...
	 * @since 1.4.0
	 */
	public synchronized boolean updateLabel(TaskEntity repository, TicketLabel label, String createdBy) {
		Repository db = null;
		try {
			db = repositoryManager.getRepository(repository.getTaskName());
			StoredConfig config = db.getConfig();
	 ...
	 * @since 1.4.0
	 */
	public synchronized boolean renameLabel(TaskEntity repository, String oldName, String newName, String createdBy) {
	 ...
	 * @since 1.4.0
	 */
	public synchronized boolean deleteLabel(TaskEntity repository, String label, String createdBy) {
	 ...
	 * @since 1.4.0
	 */
	public List<TicketMilestone> getMilestones(TaskEntity repository) {
		String key = repository.getTaskName();
	 ...
	 * @since 1.4.0
	 */
	public List<TicketMilestone> getMilestones(TaskEntity repository, Status status) {
	 ...
	 * @since 1.4.0
	 */
	public TicketMilestone getMilestone(TaskEntity repository, String milestone) {
	 ...
	 * @since 1.4.0
	 */
	public synchronized TicketMilestone createMilestone(TaskEntity repository, String milestone, String createdBy) {
	 ...
	 * @since 1.4.0
	 */
	public synchronized boolean updateMilestone(TaskEntity repository, TicketMilestone milestone, String createdBy) {
		Repository db = null;
		try {
			db = repositoryManager.getRepository(repository.getTaskName());
			StoredConfig config = db.getConfig();
	 ...
	 * @since 1.4.0
	 */
	public synchronized boolean renameMilestone(TaskEntity repository, String oldName, String newName, String createdBy) {
	 ...
	 * @since 1.6.0
	 */
	public synchronized boolean renameMilestone(TaskEntity repository, String oldName,
	 ...
	 * @since 1.4.0
	 */
	public synchronized boolean deleteMilestone(TaskEntity repository, String milestone, String createdBy) {
	 ...
	 * @since 1.6.0
	 */
	public synchronized boolean deleteMilestone(TaskEntity repository, String milestone,
	 ...
	 * @since 1.6.0
	 */
	public abstract Set<Long> getIds(TaskEntity repository);
	 ...
	 * @since 1.4.0
	 */
	public abstract long assignNewId(TaskEntity repository);
	 ...
	 * @since 1.4.0
	 */
	public abstract boolean hasTicket(TaskEntity repository, long ticketId);
	 ...
	 * @since 1.4.0
	 */
	public List<TicketModel> getTickets(TaskEntity repository) {
	 ...
	 * @since 1.4.0
	 */
	public abstract List<TicketModel> getTickets(TaskEntity repository, TicketFilter filter);
	 ...
	 * @since 1.4.0
	 */
	public final TicketModel getTicket(TaskEntity repository, long ticketId) {
		...
		if (ticket == null) {
			...
			if (ticket != null) {
				if (ticket.hasPatchsets() && updateDiffstats) {
					Repository r = repositoryManager.getRepository(repository.getTaskName());
					try {
	 ...
	 * @since 1.4.0
	 */
	protected abstract TicketModel getTicketImpl(TaskEntity repository, long ticketId);
	 ...
	 * @since 1.6.0
	 */
	public final List<Change> getJournal(TaskEntity repository, long ticketId) {
	 ...
	 * @since 1.6.0
	 */
	protected abstract List<Change> getJournalImpl(TaskEntity repository, long ticketId);
	 ...
	 * @since 1.4.0
	 */
	public abstract Attachment getAttachment(TaskEntity repository, long ticketId, String filename);
	 ...
	 * @since 1.4.0
	 */
	public TicketModel createTicket(TaskEntity repository, Change change) {
	 ...
	 * @since 1.4.0
	 */
	public final TicketModel updateTicket(TaskEntity repository, long ticketId, Change change) {
		if (change == null) {
	...
	public boolean deleteAll() {
		...
		for (int i = 0; i < repositories.size(); i++) {
			...
			TaskEntity repository = repositoryManager.getRepositoryModel(name);
	...
	public boolean deleteAll(TaskEntity repository) {
		...
		if (success) {
			log.info("Deleted all tickets for {}", repository.getTaskName());
	 ...
	 * @since 1.4.0
	 */
	protected abstract boolean deleteAllImpl(TaskEntity repository);
	 ...
	 * @since 1.4.0
	 */
	public boolean rename(TaskEntity oldRepository, TaskEntity newRepository) {
	 ...
	 * @since 1.4.0
	 */
	protected abstract boolean renameImpl(TaskEntity oldRepository, TaskEntity newRepository);
	 ...
	 * @since 1.4.0
	 */
	public boolean deleteTicket(TaskEntity repository, long ticketId, String deletedBy) {
		TicketModel ticket = getTicket(repository, ticketId);
	 ...
	 * @since 1.4.0
	 */
	protected abstract boolean deleteTicketImpl(TaskEntity repository, TicketModel ticket, String deletedBy);
...
//	}
	 ...
	 * @since 1.4.0
	 */
	protected abstract boolean commitChangeImpl(TaskEntity repository, long ticketId, Change change);
	...
	public void reindex() {
		...
		for (String name : repositoryManager.getRepositoryList()) {
			...
			if (!tickets.isEmpty()) {
				...
				log.error("failed to reindex {}", repository.getTaskName());
				log.error(null, e);
			}
		}
	 ...
	 * @since 1.4.0
	 */
	public void reindex(TaskEntity repository) {
		long start = System.nanoTime();
		List<TicketModel> tickets = getTickets(repository);
		indexer.index(tickets);
		log.info("reindexing {} tickets from {} ...", tickets.size(), repository);
