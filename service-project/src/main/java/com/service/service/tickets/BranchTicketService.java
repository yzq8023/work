...
public class BranchTicketService extends ITicketService implements RefsChangedListener {
    ...
    protected void resetCachesImpl(TaskEntity repository) {
        if (lastAssignedId.containsKey(repository.getTaskName())) {
            lastAssignedId.get(repository.getTaskName()).set(0);
        }
    }
    ...
    public synchronized long assignNewId(TaskEntity repository) {
        ...
        try {
            ...
            if (!lastAssignedId.containsKey(repository.getTaskName())) {
                lastAssignedId.put(repository.getTaskName(), new AtomicLong(0));
            }
            AtomicLong lastId = lastAssignedId.get(repository.getTaskName());
            if (lastId.get() <= 0) {
    ...
    protected synchronized boolean deleteTicketImpl(TaskEntity repository, TicketModel ticket, String deletedBy) {
        ...
        Repository db = repositoryManager.getRepository(ticket.getTaskName());
        try {
            RefModel ticketsBranch = getTicketsBranch(db);
            ...
            String ticketPath = toTicketPath(ticket.getNumber());
            ...
            try {
                ...
                success = commitIndex(db, index, deletedBy, "- " + ticket.getNumber());

            } catch (Throwable t) {
                log.error(MessageFormat.format("Failed to delete ticket {0,number,0} from {1}",
                        ticket.getNumber(), db.getDirectory()), t);
            } finally {
