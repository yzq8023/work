...
public class RedisTicketService extends ITicketService {
	 ...
	 * @return a key
	 */
	private String key(TaskEntity repository, KeyType key, String id) {
		StringBuilder sb = new StringBuilder();
		sb.append(repository.getTaskName()).append(':');
		sb.append(key.name());
	 ...
	 * @return a key
	 */
	private String key(TaskEntity repository, KeyType key, long id) {
	...
	@Override
	public Set<Long> getIds(TaskEntity repository) {
	...
	public List<TicketModel> getTickets(TaskEntity repository, TicketFilter filter) {
		...
		try {
			...
			for (String key : keys) {
				...
				ticket.setProjectName(repository.getProjectPath());
				ticket.setTaskName(repository.getTaskName());
	 ...
	 */
	private List<Change> getJournal(Jedis jedis, TaskEntity repository, long ticketId) throws JedisException {
		if (ticketId <= 0L) {
	 ...
	 */
	@Override
	protected boolean deleteTicketImpl(TaskEntity repository, TicketModel ticket, String deletedBy) {
		...
		try {
			...
			t.del(key(repository, KeyType.ticket, ticket.getNumber()));
			t.del(key(repository, KeyType.journal, ticket.getNumber()));
			t.exec();

			success = true;
			log.debug("deleted ticket {} from Redis @ {}", "" + ticket.getNumber(), getUrl());
		} catch (JedisException e) {
	 ...
	 */
	@Override
	protected boolean commitChangeImpl(TaskEntity repository, long ticketId, Change change) {
		Jedis jedis = pool.getResource();
		if (jedis == null) {
			return false;
		}
	...
	protected boolean deleteAllImpl(TaskEntity repository) {
		...
		boolean success = false;
		try {
			Set<String> keys = jedis.keys(repository.getTaskName() + ":*");
			if (keys.size() > 0) {
				Transaction t = jedis.multi();
	...
	protected boolean renameImpl(TaskEntity oldRepository, TaskEntity newRepository) {
		...
		boolean success = false;
		try {
			Set<String> oldKeys = jedis.keys(oldRepository.getTaskName() + ":*");
			Transaction t = jedis.multi();
			for (String oldKey : oldKeys) {
				String newKey = newRepository.getTaskName() + oldKey.substring(oldKey.indexOf(':'));
