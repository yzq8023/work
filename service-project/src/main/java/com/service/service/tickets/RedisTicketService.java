...
/**
 * 基于Redis键值存储的Ticket服务的实现。
 * 在Redis中保存Ticket，因此必须为其配置，否则，Ticket在重新启动时丢失。
 * Ticket与Lucene建立索引，所有查询都是针对Lucene索引执行的。
 *
 * @author James Moger
 *
 */
	...
	public List<TicketModel> getTickets(TaskEntity repository, TicketFilter filter) {
		...
		try {
			...
			for (String key : keys) {
				...
				ticket.project = repository.getProjectPath();
				ticket.repository = repository.getTaskName();
