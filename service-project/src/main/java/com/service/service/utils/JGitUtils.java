...
import com.service.service.Constants.MergeType;
...
public class JGitUtils {
	 ...
	 *
	 * @param repositoriesFolder 配置中的全路径
	 * @param name 项目名/任务名格式
	 * @param shared
	...
	public static boolean isPossibleFilestoreItem(long size) {
		return (   (size >= com.service.service.Constants.LEN_FILESTORE_META_MIN)
				&& (size <= com.service.service.Constants.LEN_FILESTORE_META_MAX));
	}
	...
	public static FilestoreModel getFilestoreItem(ObjectLoader obj){
		try {
			final byte[] blob = obj.getCachedBytes(com.service.service.Constants.LEN_FILESTORE_META_MAX);
	...
	public static List<RevCommit> searchRevlogs(Repository repository, String objectId,
                                                String value, final com.service.service.Constants.SearchType type, int offset, int maxCount) {
		List<RevCommit> list = new ArrayList<RevCommit>();
	...
	public static Map<ObjectId, List<RefModel>> getAllRefs(Repository repository, boolean includeRemoteRefs) {
		List<RefModel> list = getRefs(repository, RefDatabase.ALL, true, -1);
	...
	public static String getLfsRepositoryUrl(String baseURL, String repositoryName, String oid) {
		...
		return baseURL + com.service.service.Constants.R_PATH
					   + repositoryName + "/" 
					   + com.service.service.Constants.R_LFS
					   + "objects/" + oid;
	...
	public static boolean commitIndex(Repository db, String branch, DirCache index,
		...
		try {
			...
			commit.setEncoding(com.service.service.Constants.ENCODING);
			commit.setMessage(message);
	 ...
	 * @param repository
	 * @param settings
	 * @param baseSha, or 0 if not on a ticket branch
