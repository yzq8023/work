...
import com.service.service.utils.JGitUtils.MergeResult;
import com.service.service.utils.JGitUtils.MergeStatus;
...
/**
 * PatchsetReceivePack 处理接收指令并允许创建、更新、关闭WorkHub的ticket
 * 同时负责执行post-receive钩子
 *
...
public class PatchsetReceivePack extends GitblitReceivePack {
	...
	private boolean hasRefNamespace(String ref) {
		...
		try {
			...
			sendError("Cannot scan refs in {0}", repository.getTaskName());
			LOGGER.error("Error!", err);
			return true;
		}
		if (!blockingFors.isEmpty()) {
			sendError("{0} needs the following refs removed to receive patchsets: {1}",
					repository.getTaskName(), blockingFors.keySet());
			return true;
		}
		return false;
	}
			...
			if (isPatchsetRef(cmd.getRefName()) && processPatchsets) {
				...
				if (hasRefNamespace(Constants.R_FOR)) {
					...
					LOGGER.error("{} already has refs in the {} namespace",
							repository.getTaskName(), Constants.R_FOR);
				...
				LOGGER.info(MessageFormat.format("Verifying {0} push ref \"{1}\" received from {2}",
						repository.getTaskName(), cmd.getRefName(), user.getUsername()));
				...
				if (!StringUtils.isEmpty(responsible)) {
					...
					if (assignee == null) {
						...
						sendRejection(cmd, "{0} ({1}) can not be assigned any tickets because the user does not have RW permissions for {2}",
								assignee.getDisplayName(), assignee.getUsername(), repository.getTaskName());
						continue;
					}
				}
				...
				for (ReceiveCommand cmd : toApply) {
					if (cmd.getResult() == Result.NOT_ATTEMPTED) {
						...
						LOGGER.error(MessageFormat.format("failed to lock {0}:{1}",
								repository.getTaskName(), cmd.getRefName()), err);
					}
				}
			}
		}
			...
			for (ReceiveCommand cmd : stdUpdates) {
				...
				case UPDATE_NONFASTFORWARD:
					if (cmd.getRefName().startsWith(Constants.R_HEADS)) {
						...
						Change deletion = new Change(user.getUsername());
	...
	private PatchsetCommand preparePatchset(ReceiveCommand cmd) {
		...
		if (ticket == null) {
			if (number > 0) {
				...
				sendError("Sorry, {0} does not have ticket {1,number,0}!", repository.getTaskName(), number);
		...
		MergeStatus status = JGitUtils.canMerge(getRepository(), tipCommit.getName(), forBranch, repository.getMergeType());
		switch (status) {
		case ALREADY_MERGED:
			sendError("");
		...
		if (ticket == null) {
			...
			psCmd = new PatchsetCommand(user.getUsername(), patchset);
		...
		boolean pushPermitted = ticket == null
				|| !ticket.hasPatchsets()
				|| ticket.isAuthor(user.getUsername())
				|| ticket.isPatchsetAuthor(user.getUsername())
				|| ticket.isResponsible(user.getUsername())
			...
			while ((c = rw.next()) != null) {
				...
				for (TicketLink link : ticketLinks) {
					...
					if (link.action == TicketAction.Commit) {
						...
						change = new Change(user.getUsername());
						...
						if (knownPatchset) {
							...
							PatchsetCommand psCmd = new PatchsetCommand(user.getUsername(), patchset);
	...
	private RefUpdate updateRef(String ref, ObjectId newId, PatchsetType type) {
		...
		try {
			...
			if (result == RefUpdate.Result.LOCK_FAILURE) {
				sendError("Failed to obtain lock when updating {0}:{1}", repository.getTaskName(), ref);
			...
			sendError("There was an error updating ref {0}:{1}", repository.getTaskName(), ref);
		}
		return null;
	}
