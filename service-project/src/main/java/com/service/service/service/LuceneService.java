...
import com.service.service.Constants.SearchObjectType;
...
public class LuceneService implements Runnable {
	...
	public void run() {
		...
		for (String repositoryName: repositoryManager.getRepositoryList()) {
			TaskEntity model = repositoryManager.getRepositoryModel(repositoryName);
			if (model.isHasCommits() && !ArrayUtils.isEmpty(model.getIndexedBranches())) {
				Repository repository = repositoryManager.getRepository(model.getTaskName());
				if (repository == null) {
					if (repositoryManager.isCollectingGarbage(model.getTaskName())) {
	 ...
	 */
	private void index(TaskEntity model, Repository repository) {
		try {
			if (shouldReindex(repository)) {
				...
				if (result.success) {
					if (result.commitCount > 0) {
						...
						logger.info(MessageFormat.format(msg, model.getTaskName(), result.commitCount,
								result.blobCount, result.branchCount, result.duration()));
					}
				} else {
					String msg = "Could not build {0} Lucene index!";
					logger.error(MessageFormat.format(msg, model.getTaskName()));
				}
			} else {
			...
			logger.error(MessageFormat.format("Lucene indexing failure for {0}", model.getTaskName()), t);
		}
	}
	 ...
	 * @return IndexResult
	 */
	public IndexResult reindex(TaskEntity model, Repository repository) {
		IndexResult result = new IndexResult();
		if (!deleteIndex(model.getTaskName())) {
			return result;
		}
		try {
			...
			IndexWriter writer = getIndexWriter(model.getTaskName());
			...
			for (RefModel branch : branches) {
				...
				if (model.getIndexedBranches().contains(com.service.service.Constants.DEFAULT_BRANCH)
						&& branch.equals(defaultBranch)) {
					// indexing "default" branch
					indexBranch = true;
				} else if (branch.getName().startsWith(com.service.service.Constants.R_META)) {
					// skip internal meta branches
					indexBranch = false;
				} else {
					// normal explicit branch check
					indexBranch = model.getIndexedBranches().contains(branch.getName());
				}
			...
			config.save();
			writer.commit();
			resetIndexSearcher(model.getTaskName());
			result.success();
		} catch (Exception e) {
			logger.error("Exception while reindexing " + model.getTaskName(), e);
		}
		return result;
	}
	 ...
	 * @return IndexResult
	 */
	private IndexResult updateIndex(TaskEntity model, Repository repository) {
		IndexResult result = new IndexResult();
		try {
			FileBasedConfig config = getConfig(repository);
			config.load();
			...
			for (RefModel branch : branches) {
				...
				for (RevCommit commit : revs) {
					// index a commit
					result.add(index(model.getTaskName(), repository, branchName, commit));
				}
			...
			logger.error(MessageFormat.format("Exception while updating {0} Lucene index", model.getTaskName()), t);
		}
		return result;
	}
