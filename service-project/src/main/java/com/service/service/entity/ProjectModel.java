...
public class ProjectModel implements Serializable, Comparable<ProjectModel> {
	...
	public void addRepository(TaskEntity model) {
		repositories.add(model.getTaskName().toLowerCase());
		if (lastChange.before(model.getUpdTime())) {
			lastChange = model.getUpdTime();
		}
	}
