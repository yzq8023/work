...
public class TicketModel implements Serializable, Comparable<TicketModel> {
    ...
    /**
     * 从changes集合中构建有效的票据Ticket。 一个change可能从Ticket中添加或减去信息,
     *
     * @param changes
    ...
    public TicketModel() {
        ...
        type = String.valueOf(Type.defaultType);
        priority = String.valueOf(Priority.defaultPriority);
        severity = String.valueOf(Severity.defaultSeverity);
    }

    public boolean isOpen() {
        return !Status.valueOf(status).isClosed();
    }
    ...
    public boolean isProposal() {
        return Type.Proposal == Type.valueOf(type);
    }

    public boolean isBug() {
        return Type.Bug == Type.valueOf(type);
    }

    public Date getLastUpdated() {
        return updTime == null ? crtTime : updTime;
    }
    ...
    public boolean hasDiscussion() {
        for (Change change : getComments()) {
            if (!change.author.equals(crtUser)) {
                return true;
            }
        }
        return false;
    ...
    public boolean isAuthor(String username) {
        return username.equals(crtUser);
    }
    ...
    public void applyChange(Change change) {
        if (changes.size() == 0) {
            ...
            crtTime = change.date;
            crtUser = change.author;
            status = String.valueOf(Status.New);
        } else if (crtTime == null || change.date.after(crtTime)) {
            // track last ticket update
            updTime = change.date;
            updUser = change.author;
        }
        ...
        if (change.isMerge()) {
            ...
            status = String.valueOf(Status.Merged);
        }
    ...
    public int compareTo(TicketModel o) {
        return o.crtTime.compareTo(crtTime);
    }
    ...
    @Override
    public int hashCode() {
        return (taskName + number).hashCode();
    }
