...
import com.service.service.utils.ObjectCache;
...
public class ProjectManager implements IProjectManager {
	...
	private void reloadProjectMarkdown(ProjectModel project) {
		...
		if (pmkd.exists()) {
			...
			if (!projectMarkdownCache.hasCurrent(project.name, lm)) {
				String mkd = com.service.service.utils.FileUtils.readContent(pmkd,  "\n");
		...
		if (rmkd.exists()) {
			...
			if (!projectRepositoriesMarkdownCache.hasCurrent(project.name, lm)) {
				String mkd = com.service.service.utils.FileUtils.readContent(rmkd,  "\n");
	...
	public List<ProjectModel> getProjectModels(UserModel user, boolean includeUsers) {
		...
		for (TaskEntity model : repositoryManager.getRepositoryModels(user)) {
			String projectPath = StringUtils.getRootPath(model.getTaskName());
	...
	public List<ProjectModel> getProjectModels(List<TaskEntity> repositoryModels, boolean includeUsers) {
		...
		for (TaskEntity repository : repositoryModels) {
			if (!includeUsers && repository.isPersonalRepository()) {
				// exclude personal repositories
				continue;
			}
			if (!projects.containsKey(repository.getProjectPath())) {
				ProjectModel project = getProjectModel(repository.getProjectPath());
				if (project == null) {
					logger.warn(MessageFormat.format("excluding project \"{0}\" from project list because it is empty!",
							repository.getProjectPath()));
					continue;
				}
				projects.put(repository.getProjectPath(), project);
				...
				project.repositories.add(repository.getTaskName());
				project.lastChange = repository.getUpdTime();
			} else {
				// update the user-accessible list
				// this is used for repository count
				ProjectModel project = projects.get(repository.getProjectPath());
				project.repositories.add(repository.getTaskName());
				if (project.lastChange.before(repository.getUpdTime())) {
					project.lastChange = repository.getUpdTime();
				}
			}
		}
